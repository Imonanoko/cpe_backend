...
async fn excel_search_pass(
    ...
    while let Some(Ok(field)) = payload.next().await {
        ...
        if let Some(_filename) = content_disposition.and_then(|cd| cd.get_filename()) {
    ...
    let mut workbook = match calamine::open_workbook_auto(temp_filepath) {
        ...
        Err(err) => {
            ...
            return HttpResponse::InternalServerError().body("Invalid Excel file");
        }
    };

    let range = match workbook.worksheet_range("工作表1") {
        Ok(range) => range, // 成功时返回 range
        Err(err) => {
            println!("Error reading sheet: {}", err);
            return HttpResponse::BadRequest().body("Invalid Excel sheet");
        }
    };

    let student_ids: Vec<String> = range
        .rows()
        .skip(1)
    ...
    if student_ids.is_empty() {
        return HttpResponse::BadRequest().body("No valid Student IDs found in Excel file");
    }
    let query = r#"
    SELECT 
    ...
    worksheet
        .write_string(0, 1, "姓名", None)
        .unwrap();
    worksheet
        .write_string(0, 2, "累計題數", None)
        .unwrap();
    worksheet
        .write_string(0, 3, "最高題數", None)
        .unwrap();
    worksheet.write_string(0, 4, "是否通過", None).unwrap();
    for (i, (student_id,name, total_correct_answers, max_correct_answers)) in result.iter().enumerate() {
        worksheet
